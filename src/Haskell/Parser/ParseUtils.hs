...
module Haskell.Parser.ParseUtils (
	...
	, mkAndType             -- HsType -> HsType -> HsType
 ) where
...
-- Intersection Types construction
mkAndType :: HsType -> HsType -> HsType
mkAndType (HsTyAnd l) (HsTyAnd l') = HsTyAnd (l ++ l)
mkAndType t (HsTyAnd l) = HsTyAnd (t:l)
mkAndType (HsTyAnd l) t = HsTyAnd (l ++ [t])
mkAndType t1 t2 = HsTyAnd [t1,t2]
